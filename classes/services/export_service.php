...
class export_service {
     ...
     */
    public function __construct($migrationmode = constants::MIGRATION_MODE_DISABLED) {
    ...
    private function setup_migration_params($datatype) {
        ...
        if ($this->migrationmode == constants::MIGRATION_MODE_ENABLED) {
            $datatype['name'] = $this->get_migration_name($datatype);
        }

        $datatype['migrationmode'] = (!empty($datatype['databaseexport']))
             ? constants::MIGRATION_MODE_ENABLED : $this->migrationmode;
